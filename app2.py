...
@st.cache_data
def load_data():
    df_main = pd.read_csv('/Users/apple/Desktop/stream_deployed/data/finalist_data.csv')
...
#  Code for use the vector database but the problem is it is not saved in the local directory.
@st.cache_resource
...
def get_vehicle_comparison_data(query, df, names_set):
    ...
    parts = re.split(r'\s+vs\s+|\s+\s+', query.lower())
    if len(parts) != 2:
...
def process_query(prompt: str):
    ...
    # 1. Substring match: find all vehicles whose normalized name contains the normalized query
    ...
    if qna_answer:
        with st.spinner("Enhancing answer..."):
            enhance_prompt = f"Rewrite and enhance the following answer for the question: '{prompt}'.\nAnswer: {qna_answer}\nMake it more helpful, clear, and engaging, but keep it accurate and relevant."
            result = qa_chain.invoke({"question": enhance_prompt, "context": qna_answer})
            enhanced_answer = result.get("result") or qna_answer
            enhanced_answer = clean_llm_output(enhanced_answer)
        with st.chat_message("assistant"):
