...
#     if pd.isnull(price):
#         return None
#     price_str = str(price).lower().replace('₹', '').replace(',', '').replace('lakh', '').strip()
#     if any(x in price_str for x in ['coming soon', 'n/a', 'na', 'not available', 'nan', '']):
#         return None
#     try:
...
# TRUCK_CATEGORIES = {"truck", "pickup truck", "light truck", "heavy truck", "mini truck", "tipper truck", "cargo truck", "lorry"}

# def filter_vehicles_by_brand_category_price(df, brand=None, category=None, min_price=None, max_price=None, n=None):
#     filtered = df.copy()
#     if brand:
#         filtered = filtered[filtered['Brand Name'].str.lower() == brand.lower()]
#     if category:
#         category_cols = ['Category Name', 'category_name', 'Vehicle Type', 'Category']
#         cat_mask = pd.Series([False] * len(filtered), index=filtered.index)
#         for col in category_cols:
#             if col in filtered.columns:
#                 # Stricter matching for 'truck' category
#                 if category.lower() == 'truck':
#                     cat_mask = cat_mask | filtered[col].str.lower().isin([c for c in TRUCK_CATEGORIES])
#                 else:
#                     cat_mask = cat_mask | filtered[col].str.lower().str.contains(category.lower(), na=False)
#         filtered = filtered[cat_mask]
#     filtered = filtered.copy()
#     filtered['numeric_price'] = filtered['Price'].apply(extract_numeric_price)
#     filtered = filtered[filtered['numeric_price'].notnull()]
#     if min_price is not None:
#         filtered = filtered[filtered['numeric_price'] >= min_price]
#     if max_price is not None:
#         filtered = filtered[filtered['numeric_price'] <= max_price]
#     if n is not None:
...
#                 recs = get_recommendations(data, df_main)
#                 if category_slug:
#                     if category_slug == "truck":
#                         recs = recs[[str(row.get(col, '')).lower() in TRUCK_CATEGORIES for _, row in recs.iterrows()]]
#                     else:
#                         recs = recs[[category_slug in str(row.get(col, '')).lower() for _, row in recs.iterrows()]]
#                 if not recs.empty:
#                     st.subheader("Recommended Alternatives:")
#                     rec_datas = [get_vehicle_data(rec_row['Vehicle Name'], df_main) for _, rec_row in recs.iterrows()]
#                     cols = st.columns(len(rec_datas))
...
#     # --- Handle '<brand> <category> between <min> to <max> lakh(s)' queries ---
#     price_range_match = re.match(r"([a-zA-Z]+)\s+([a-zA-Z]+)s?\s+between\s+(\d+)\s+to\s+(\d+)\s+lakh[s]?", prompt_lower)
#     if price_range_match:
#         brand = price_range_match.group(1)
#         category = price_range_match.group(2)
#         min_price = float(price_range_match.group(3))
#         max_price = float(price_range_match.group(4))
#         filtered_df = filter_vehicles_by_brand_category_price(df_main, brand=brand, category=category, min_price=min_price, max_price=max_price)
#         if filtered_df.empty:
#             st.warning(f"Sorry, I couldn't find any {brand.title()} {category}s between {int(min_price)} and {int(max_price)} lakh with a valid price in my database.")
#             return
#         st.subheader(f"{brand.title()} {category.title()}s between ₹{int(min_price)} and ₹{int(max_price)} Lakh:")
...
#     # --- Handle '<category> under <price> lakh(s)' queries ---
#     price_cat_match = re.match(r"([a-zA-Z]+)s?\s+under\s+(\d+)\s+lakh[s]?", prompt_lower)
#     if price_cat_match:
#         category = price_cat_match.group(1)
#         max_price = float(price_cat_match.group(2))
#         filtered_df = filter_vehicles_by_brand_category_price(df_main, category=category, max_price=max_price)
#         if filtered_df.empty:
#             st.warning(f"Sorry, I couldn't find any {category}s under {int(max_price)} lakh with a valid price in my database.")
#             return
#         st.subheader(f"{category.title()}s under ₹{int(max_price)} Lakh:")
#         for _, row in filtered_df.iterrows():
#             data = get_vehicle_data(row['Vehicle Name'], df_main)
#             if data:
#                 display_vehicle_card(data)
...
#     # --- Handle 'show N <brand> <category>' queries ---
#     show_n_brand_cat_match = re.match(r"show\s+(\d+)\s+([a-zA-Z]+)\s+([a-zA-Z]+)s?", prompt_lower)
#     if show_n_brand_cat_match:
#         n = int(show_n_brand_cat_match.group(1))
#         brand = show_n_brand_cat_match.group(2)
#         category = show_n_brand_cat_match.group(3)
#         filtered_df = filter_vehicles_by_brand_category_price(df_main, brand=brand, category=category, n=n)
#         if filtered_df.empty:
#             st.warning(f"Sorry, I couldn't find any {brand.title()} {category}s in my database.")
#             return
#         st.subheader(f"Showing up to {n} {brand.title()} {category.title()}s:")
#         for _, row in filtered_df.iterrows():
#             data = get_vehicle_data(row['Vehicle Name'], df_main)
#             if data:
#                 display_vehicle_card(data)
...
#         response = "I'm your commercial vehicle assistant for 91Trucks. I can help you with trucks, buses, autos, and related queries. Please ask about vehicles that are available in our dataset or listed on 91Trucks."
#         with st.chat_message("assistant"):
#             st.markdown(response)
#         st.session_state.chat_history.append({"role": "assistant", "content": response})
#         return
...
def clean_price(price):
    if isinstance(price, str):
        ...
        numbers = re.findall(r'(\d+\.?\d*)', price_lower)
        if not numbers:
            return None
        ...
        if 'lakh' in price:
            ...
            return value * 100
    # Handle cases where the price is already a float or int
...
def load_data():
    ...
    df_qna = pd.read_csv('data/91Trucks_QnAs.csv')
    df_main["Vehicle Image"] = df_main["Vehicle Image"].fillna("")
    # This creates the 'Price' column with cleaned numeric values (in Lakhs) or 'Coming Soon'
    df_main["Price"] = df_main["Vehicle Price"].apply(clean_price)
    return df_main, df_qna
...
def get_vector_store(_df_main, _df_qna):
    with st.spinner("Initializing knowledge base... this may take a moment."):
        ...
        for _, row in _df_main.iterrows():
            price_val = row['Price']
            if pd.notnull(price_val) and isinstance(price_val, (int, float)):
                price_str = f"{price_val} Lakh"
            elif isinstance(price_val, str):
                price_str = price_val # Handles "Coming Soon"
            else:
                price_str = "N/A"
...
def get_vehicle_data(display_name, df):
    display_name_clean = display_name.lower().strip()
    # Use exact match first for precision
    vehicle_data_df = df[df['Vehicle Name'].str.lower().str.strip() == display_name_clean]
    # Fallback to contains if no exact match is found
    if vehicle_data_df.empty:
        vehicle_data_df = df[df['Vehicle Name'].str.lower().str.strip().str.contains(display_name_clean, na=False)]

    if not vehicle_data_df.empty:
        # Aggregate data from all matching rows (variants)
        prices = vehicle_data_df['Price'].dropna().tolist()
        numeric_prices = [p for p in prices if isinstance(p, (int, float))]
        coming_soon = any(isinstance(p, str) and 'coming soon' in p.lower() for p in prices)
        ...
        # Take the first row for non-aggregated data
        row = vehicle_data_df.iloc[0].to_dict()
        data = {
            "Vehicle Name": row['Vehicle Name'],
            "Brand Name": row['Brand Name'],
            ...
            "Average Rating": row.get('Average Rating', "Not available"),
            "End Point": row.get('End Point'),
            "Price": price_str,            # Formatted string for display
            "Numeric Price": numeric_prices, # List of numbers for calculation
            "Fuel Type": fuel_types_str,
            "Power": power_str,
            "Variant Name": variants_str,
            "Pros": pros_str if pros_str else "Not available",
...
def display_faq_sidebar(df_qna=None):
    ...
    questions = [
        ...
        "how can i contact to the 91trucks",
        "How 91trucks can help me to get good vehicle",
        "compare tata intra v30 vs tata ace gold",
        "compare mahindra jeeto plus vs mahindra supro profit truck mini",
        "tata trucks",
        "eicher trucks",
        "What is the primary focus of 91Trucks' digital platform?",
        "What is the significance of 91Trucks' physical retail stores?",
        ...
        for col in category_cols:
            if col in filtered.columns:
                ...
                if category.lower() == 'truck':
                    ...
                    cat_mask = cat_mask | filtered[col].str.lower().str.contains(category.lower(), na=False)
        filtered = filtered[cat_mask]
    # Use the 'Price' column which already has numeric values
    filtered = filtered.copy()
    filtered = filtered[pd.to_numeric(filtered['Price'], errors='coerce').notnull()]
    filtered['numeric_price'] = pd.to_numeric(filtered['Price'], errors='coerce')
...
def get_recommendations(main_vehicle, df, n=3):
    """
    Finds and returns recommended vehicles based on price and category.
    - main_vehicle: The data dictionary of the vehicle to base recommendations on.
    - df: The main DataFrame of all vehicles.
    - n: The number of recommendations to return.
    """
    main_vehicle_name = main_vehicle.get("Vehicle Name")
    numeric_prices = main_vehicle.get("Numeric Price") # This is a list of prices

    # 1. Check if there's a valid price to base recommendations on
    if not numeric_prices:
        return pd.DataFrame() # Return empty DataFrame if no price

    # Use the average price for comparison
    base_price = sum(numeric_prices) / len(numeric_prices)

    # 2. Define a price range (e.g., +/- 15%)
    price_margin = 0.15
    min_price = base_price * (1 - price_margin)
    max_price = base_price * (1 + price_margin)

    # 3. Filter the DataFrame
    # Ensure we only work with vehicles that have a numeric price
    recs_df = df[pd.to_numeric(df['Price'], errors='coerce').notnull()].copy()
    recs_df['numeric_price'] = pd.to_numeric(recs_df['Price'])

    # Exclude the main vehicle itself
    recs_df = recs_df[recs_df['Vehicle Name'] != main_vehicle_name]

    # Filter by price range
    recs_df = recs_df[(recs_df['numeric_price'] >= min_price) & (recs_df['numeric_price'] <= max_price)]

    # 4. (Optional but recommended) Prioritize vehicles in the same category
    main_category = main_vehicle.get("Category Name")
    if main_category and isinstance(main_category, str):
        # Find other vehicles in the same category
        same_category_recs = recs_df[recs_df['Category Name'].str.lower() == main_category.lower()]
        # Find vehicles from other categories (to fill up if needed)
        other_category_recs = recs_df[recs_df['Category Name'].str.lower() != main_category.lower()]
        # Combine them, prioritizing the same category
        final_recs = pd.concat([same_category_recs, other_category_recs])
    else:
        # If no category, just use the price-filtered list
        final_recs = recs_df

    # 5. Remove duplicates and return the top N results
    final_recs = final_recs.drop_duplicates(subset=['Vehicle Name'])
...
def process_query(prompt: str):
    ...
    if found_vehicle:
        ...
        if prompt_lower.strip() == found_vehicle.lower().strip():
            data = get_vehicle_data(found_vehicle, df_main)
            if not data:
                 st.warning(f"Sorry, I couldn't find details for '{found_vehicle}' in my database.")
                 return
    ...
    for norm_name, orig_name in normalized_display_names.items():
        ...
        scores.append((score, orig_name))
    scores.sort(reverse=True)
    best_score, best_name = scores[0] if scores else (0, None)

    if best_score > 0.85:
        data = get_vehicle_data(best_name, df_main)
        st.subheader(f"Showing results for: {best_name}")
        if data:
            display_vehicle_card(data)
        else:
            st.warning(f"I'm sorry, I don't have details for '{best_name}' in my current database.")
    ...
    if price_cat_match:
        category = price_cat_match.group(1)
        max_price = float(price_cat_match.group(2))
        filtered_df = filter__vehicles_by_brand_category_price(df_main, category=category, max_price=max_price)
        if filtered_df.empty:
            st.warning(f"Sorry, I couldn't find any {category}s under {int(max_price)} lakh with a valid price in my database.")
            return
        st.subheader(f"{category.title()}s under ₹{int(max_price)} Lakh:")
